                    ...
                    case "setstate":
                        switch (event.data.stateKey) {
                            case Constants.allScoresStateKey: {
                                ...
                                // OVERRIDE: honor operator-configured "None" in
                                // overrides/games.json. Upstream unconditionally
                                // overwrites highScoreMode with whatever the
                                // running game posts; we suppress that overwrite
                                // when the kiosk admin explicitly opted out of
                                // scoring. See ScoringType-casing bug analysis
                                // for the full chain.
                                if (
