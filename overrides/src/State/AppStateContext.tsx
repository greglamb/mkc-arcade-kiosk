...
export function AppStateProvider(
    ...
    // OVERRIDE: this kiosk is admin-curated via overrides/games.json, so
    // `locked` defaults to true (hiding the "Add your game" button + the
    // user-added-game removal UI). The original upstream line was:
    //   const locked = !!/locke?d?(?:[:=])1/i.test(url);
    // We invert the sense so `?locked=0` (or `?lock=0`, etc.) is the
    // debug escape hatch to UNLOCK the kiosk for testing.
    const locked = !/locke?d?(?:[:=])0/i.test(url);
